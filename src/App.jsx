...
export default function App(){
 ...
 const stats=useMemo(()=>{const meetings=slots.filter(s=>s.event).length,open=slots.filter(s=>s.event&&!s.owner).length,covered=slots.filter(s=>s.event&&s.owner).length,mine=slots.filter(s=>s.event&&s.owner==='Dharshni Rangarajan').length;return{meetings,open,covered,mine,coverage:meetings===0?0:Math.round(covered/meetings*100)}},[slots])
 ...
 <div className="summary-grid">
  <div className="card summary"><span>Meetings</span><strong>{stats.meetings}</strong></div>
  <div className="card summary"><span>Needs cover</span><strong className="red">{stats.open}</strong></div>
  <div className="card summary"><span>Covered</span><strong className="green">{stats.covered}</strong></div>
  <div className="card summary"><span>My meetings</span><strong>{stats.mine}</strong></div>
  <div className="card summary"><span>Coverage</span><strong>{stats.coverage}%</strong></div>
 </div>
